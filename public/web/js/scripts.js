		...
		(function () {
				"use strict";
                    ...
                    function clearPasswordErrors() {
                        var _form = $('#form-password-change');
                        _form.find('.has-error').removeClass('has-error has-feedback');
                        _form.find('.validate-has-error').remove();
                        _form.find('.form-control-feedback').remove();
                        _form.find('.validateCreditCard').tooltip('destroy');
                    }

					function submitPasswordChange() {
						var _form = $('#form-password-change');
						var _data = _form.serialize();
						_form.find('input, textarea, button, a, select').prop('disabled', true);
						$.ajax({
							...
							success: function (data) {
								...
								if (data.status == 'success') {
                                    _form.find('input, textarea, button, a, select').prop('disabled', false);
									$('#modal_password_change').modal('hide');
									$('#modal_password_changed').modal();

								} else {
									_form.find('input, textarea, button, a, select').prop('disabled', false);
									$.each((data.errors), function (i, e) {
										$('#' + i).parents('.form-group').addClass('has-error');
										$('#' + i).after('<span class="validate-has-error">' + e + '</span>');
									});
								}
							}
						})
